/**
 * @file navier_stokes_pcd_viscosity_sweep_afepack.cpp
        ...
        for (; picard_iteration < max_picard_iterations; ++picard_iteration)
          {
            ...
            if (!gmres_info.converged)
              throw std::runtime_error(
                "right-preconditioned PCD GMRES failed for viscosity " +
  ...
  const std::string output_prefix =
    argc > 2 ? argv[2] :
               std::string(AFEPACK_DEFAULT_OUTPUT_DIR) +
                 "/navier_stokes_pcd_viscosity_sweep_afepack";
  ...
  try
    {
      ...
      std::cout << "AFEPack Navier-Stokes Picard PCD viscosity continuation "
