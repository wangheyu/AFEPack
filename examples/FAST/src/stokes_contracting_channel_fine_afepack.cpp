/**
 * @file stokes_contracting_channel_fine_afepack.cpp
...
namespace
{
  ...
  struct ContractingFineRow
  {
    ContractingFineCase     mesh_case;
    std::string             method;
    ContractingChannelResult result;
  };

  ContractingFineCase default_mesh_case()
  {
    const std::string mesh_dir = AFEPACK_DEFAULT_MESH_DIR;
    const std::string output_dir = AFEPACK_DEFAULT_OUTPUT_DIR;
    return {"contracting_channel_h0p125",
            mesh_dir + "/contracting_channel_h0p125",
            output_dir + "/stokes_contracting_channel_fine_h0p125",
            0.125};
  }
  ...
  void print_result_row(const ContractingFineRow &row)
  {
  ...
  try
    {
      ...
      const std::vector<ContractingFineRow> rows = {
        {mesh_case,
         "P2P1_TaylorHood",
         solve_contracting_channel(mesh_case.mesh_file,
                                   mesh_case.output_prefix + "_p2p1",
                                   2,
                                   1,
                                   false)},
        {mesh_case,
         "P1P1_LPS",
         solve_contracting_channel(mesh_case.mesh_file,
                                   mesh_case.output_prefix + "_p1p1_lps",
                                   1,
                                   1,
                                   true)}};

      std::cout << std::scientific << std::setprecision(6);
      std::cout << "AFEPack Stokes contracting-channel fine grid "
