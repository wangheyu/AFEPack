/**
 * @file oseen_block_lsc_afepack.cpp
 * @brief AFEPack Oseen 线性化方程算例：LSC 预条件器。
 *
 * @details
...
namespace
{
  ...
  OseenLSCResult solve_oseen_block_lsc(const std::string &mesh_file,
  ...
  const std::string output_prefix =
    argc > 2 ? argv[2] :
               std::string(AFEPACK_DEFAULT_OUTPUT_DIR) +
                 "/oseen_block_lsc_afepack";

  try
    {
      const OseenLSCResult result =
        solve_oseen_block_lsc(mesh_file, output_prefix);

      std::cout << std::scientific << std::setprecision(6);
      std::cout << "AFEPack Oseen block LSC Taylor-Hood manufactured "
                   "solution\n";
