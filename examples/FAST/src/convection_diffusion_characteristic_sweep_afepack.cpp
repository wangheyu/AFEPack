/**
 * @file convection_diffusion_characteristic_sweep_afepack.cpp
...
#define main convection_diffusion_characteristic_hidden_main
#include "convection_diffusion_characteristic_layer_afepack.cpp"
#undef main
...
namespace
{
  ...
  enum class CharacteristicPreconditioner
  {
    none,
    jacobi,
    downstream_gs,
  ...
  struct CharacteristicSweepResult
  {
  ...
  const char *
  preconditioner_name(const CharacteristicPreconditioner preconditioner)
  {
    switch (preconditioner)
      {
        case CharacteristicPreconditioner::none:
          return "none";
        case CharacteristicPreconditioner::jacobi:
          return "Jacobi";
        case CharacteristicPreconditioner::downstream_gs:
          return "bottom-to-top-GS";
        case CharacteristicPreconditioner::upstream_gs:
          return "top-to-bottom-GS";
        case CharacteristicPreconditioner::symmetric_y_gs:
          return "symmetric-y-GS";
      }
    return "unknown";
  }
                             ...
                             const SweepDirection direction)
  {
    ...
    std::sort(order.begin(),
              ...
              [&](const int left, const int right) {
                ...
                if (std::abs(pl[1] - pr[1]) > 1.0e-12)
                  {
                    const bool increasing =
                      direction == SweepDirection::bottom_to_top;
                    return increasing ? pl[1] < pr[1] : pl[1] > pr[1];
                  }
                if (std::abs(pl[0] - pr[0]) > 1.0e-12)
                  return pl[0] < pr[0];
                return left < right;
              });
    return order;
  ...
  CharacteristicSweepResult run_characteristic_sweep_case(
    const std::string &mesh_file,
    const double h,
    const double epsilon,
    const CharacteristicPreconditioner preconditioner)
  {
    ...
    BoundaryFunction<double, dim> inflow_boundary(
      BoundaryConditionInfo::DIRICHLET, 1, &boundary_inflow);
    BoundaryFunction<double, dim> right_wall_boundary(
      BoundaryConditionInfo::DIRICHLET, 2, &boundary_right_wall);
    BoundaryFunction<double, dim> left_wall_boundary(
      BoundaryConditionInfo::DIRICHLET, 4, &boundary_left_wall);

    BoundaryConditionAdmin<double, dim> boundary_admin(fem_space);
    boundary_admin.add(inflow_boundary);
    boundary_admin.add(right_wall_boundary);
    boundary_admin.add(left_wall_boundary);
    boundary_admin.apply(matrix, solution, rhs);

    for (std::size_t i = 0; i < solution.size(); ++i)
      solution(i) = 0.0;

    DirectionalGaussSeidelPreconditioner downstream_sweep(
      matrix,
      {dof_order(fem_space, SweepDirection::bottom_to_top)});
    DirectionalGaussSeidelPreconditioner upstream_sweep(
      matrix,
      {dof_order(fem_space, SweepDirection::top_to_bottom)});
    DirectionalGaussSeidelPreconditioner symmetric_sweep(
      matrix,
      {dof_order(fem_space, SweepDirection::bottom_to_top),
       dof_order(fem_space, SweepDirection::top_to_bottom)});
  ...
  void print_results(const std::vector<CharacteristicSweepResult> &results)
  {
    std::cout << std::scientific << std::setprecision(6);
    std::cout << "AFEPack characteristic boundary-layer SUPG sweep "
                 "preconditioner test\n";
    std::cout << "Book Example 3.1.2; wind is vertical and positive, "
                 "so bottom-to-top is the downstream sweep.\n";
  ...
  try
    {
      ...
      const double epsilon = 1.0 / 200.0;
      const std::vector<CharacteristicPreconditioner> preconditioners = {
        CharacteristicPreconditioner::none,
        CharacteristicPreconditioner::jacobi,
        CharacteristicPreconditioner::downstream_gs,
        CharacteristicPreconditioner::upstream_gs,
        CharacteristicPreconditioner::symmetric_y_gs};

      std::vector<CharacteristicSweepResult> results;
      for (const auto preconditioner : preconditioners)
        results.push_back(run_characteristic_sweep_case(mesh_file,
                                                        0.05,
                                                        epsilon,
                                                        preconditioner));
