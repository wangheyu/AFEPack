/**
 * @file navier_stokes_lsc_iterative_afepack.cpp
...
namespace
{
  ...
  constexpr double picard_tolerance = 1.0e-6;
  ...
  NavierStokesLSCResult solve_navier_stokes_lsc(
    ...
    if (final_relative_update >= picard_tolerance)
      {
        std::ostringstream message;
        message << std::scientific << std::setprecision(6)
                << "Picard iteration did not converge within "
  ...
  void print_result_table(const std::vector<NavierStokesLSCResult> &results)
  {
    std::cout << "preconditioner cells velocity_dofs pressure_dofs Picard "
                 "GMRES_total final_update final_gmres_residual velocity_L2 "
  ...
  const std::string output_prefix =
    argc > 2 ? argv[2] :
               std::string(AFEPACK_DEFAULT_OUTPUT_DIR) +
                 "/navier_stokes_lsc_iterative_afepack";

  try
    {
      std::cout << std::scientific << std::setprecision(6);
      std::cout << "AFEPack Navier-Stokes Picard LSC with iterative inner "
                   "solvers Taylor-Hood manufactured solution\n";

      std::vector<NavierStokesLSCResult> results;
      results.push_back(
        solve_navier_stokes_lsc(mesh_file,
                                output_prefix,
