/**
 * @file navier_stokes_step_flow_equal_order_stabilized_afepack.cpp
...
#define main navier_stokes_step_flow_afepack_hidden_main
#include "navier_stokes_step_flow_afepack.cpp"
#undef main

namespace
{
  constexpr double equal_order_stabilization_parameter = 1.0;

  struct StabilizedNavierStokesStepResult
  {
                ...
                for (int j = 0; j < n_local_pressure_dofs; ++j)
                  {
                    ...
                    add_entry(matrix,
                              ...
                              pressure_index(pi_dof, n_velocity_dofs),
                              pressure_index(pj, n_velocity_dofs),
                              -equal_order_stabilization_parameter * jxw *
  ...
  StabilizedNavierStokesStepResult solve_stabilized_viscosity_step(
    const EasyMesh &mesh,
    const FEMSpace<double, dim> &velocity_space,
    const FEMSpace<double, dim> &pressure_space,
    const double viscosity,
  ...
  std::vector<StabilizedNavierStokesStepResult>
  solve_stabilized_step_flow_continuation(
    ...
    const std::vector<double> &viscosities)
  {
    ...
    std::vector<StabilizedNavierStokesStepResult> results;
    results.reserve(viscosities.size());

    for (const double viscosity : viscosities)
      results.push_back(solve_stabilized_viscosity_step(mesh,
                                                        velocity_space,
                                                        pressure_space,
                                                        viscosity,
                                                        outlet_x,
...
#ifndef AFEPACK_NAVIER_STOKES_STEP_FLOW_EQUAL_ORDER_NO_MAIN
int main(int argc, char **argv)
{
  ...
  const std::string output_prefix =
    argc > 2 ? argv[2] :
               std::string(AFEPACK_DEFAULT_OUTPUT_DIR) +
                 "/navier_stokes_step_flow_equal_order_stabilized_afepack";
  ...
  try
    {
      const auto results =
        solve_stabilized_step_flow_continuation(mesh_file,
                                                output_prefix,
                                                viscosities);

      std::cout << std::scientific << std::setprecision(6);
      std::cout << "AFEPack Navier-Stokes P1/P1 local-projection "
                   "stabilized expansion step-flow continuation "
                   "(book Example 7.1.2)\n";
      std::cout << "pressure local-projection stabilization parameter beta="
                << equal_order_stabilization_parameter << '\n';
      std::cout << "nu Re cells velocity_dofs pressure_dofs Picard "
                   "rel_update velocity_L2 velocity_H1 max_speed "
                   "inflow_rate outflow_rate flux_imbalance pressure_min "
                   "pressure_max pressure_mean divergence_L2 "
                   "wall_shear_samples reattachment_found reattachment_x "
                   "wall_shear_min wall_shear_max outlet_x\n";
      for (const auto &r : results)
        {
          std::cout << r.viscosity << ' '
                    << r.reynolds_number << ' '
                    << r.cells << ' '
                    << r.velocity_dofs << ' '
                    << r.pressure_dofs << ' '
                    ...
                    << r.wall_shear_samples << ' '
                    << r.reattachment_found << ' '
                    << r.reattachment_x << ' '
                    << r.wall_shear_min << ' '
                    << r.wall_shear_max << ' '
                    << r.outlet_x << '\n';
