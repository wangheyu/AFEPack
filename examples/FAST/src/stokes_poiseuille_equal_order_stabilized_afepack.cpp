/**
 * @file stokes_poiseuille_equal_order_stabilized_afepack.cpp
...
namespace
{
  ...
  struct StabilizedPoiseuilleResult
  {
    double       h = 0.0;
    int          cells = 0;
  ...
  StabilizedPoiseuilleResult solve_stabilized_poiseuille(
  ...
  try
    {
      ...
      std::cout << "AFEPack Stokes P1/P1 local-projection stabilized "
                   "Poiseuille test\n";
      std::cout << "Book Example 5.1.1 on the unit-square channel; "
          ...
          const std::string output_prefix =
            argc > 2 ? argv[2] :
                       std::string(AFEPACK_DEFAULT_OUTPUT_DIR) +
                         "/stokes_poiseuille_equal_order_stabilized_afepack";
          const StabilizedPoiseuilleResult result =
            solve_stabilized_poiseuille(mesh_file, output_prefix, 0.0);
      ...
      std::vector<StabilizedPoiseuilleResult> results;
      for (const auto &[mesh_name, h] : meshes)
        {
          ...
          const std::string output_prefix =
            std::string(AFEPACK_DEFAULT_OUTPUT_DIR) +
            "/stokes_poiseuille_equal_order_stabilized_afepack_" +
            mesh_name;
          results.push_back(solve_stabilized_poiseuille(mesh_file,
                                                        output_prefix,
                                                        h));
        }

      std::cout << "h cells velocity_dofs pressure_dofs "
                   "velocity_L2 velocity_L2_rate velocity_H1 "
