/**
 * @file oseen_block_lsc_iterative_afepack.cpp
...
namespace
{
  ...
  struct OseenLSCIterativeResult
  {
  ...
  OseenLSCIterativeResult solve_oseen_block_lsc_iterative(
    ...
    const VelocityInverseKind velocity_inverse_kind)
  {
    ...
    BlockLSCPreconditioner preconditioner(n_velocity_dofs,
                                          n_pressure_dofs,
                                          system.velocity_block,
                                          system.divergence,
                                          system.velocity_mass_diagonal,
                                          velocity_amg.get(),
                                          velocity_inverse_kind ==
                                            VelocityInverseKind::
                                              amg_preconditioned_oseen);
    ...
    return {preconditioner_name(velocity_inverse_kind),
  ...
  void print_result_table(const std::vector<OseenLSCIterativeResult> &results)
  {
    std::cout << "preconditioner cells velocity_dofs pressure_dofs GMRES "
                 "status "
                 "gmres_residual velocity_L2 "
  ...
  void print_sweep_row(const MeshSweepCase &mesh_case,
                       const OseenLSCIterativeResult &result)
  {
    std::cout << result.preconditioner << ' '
              << mesh_case.h << ' '
              << result.cells << ' '
  ...
  void run_default_mesh_sweep()
  {
    ...
    const std::string output_stem =
      output_dir + "/oseen_block_lsc_iterative_afepack";

    std::cout << "AFEPack Oseen block LSC with iterative inner solvers "
                 "Taylor-Hood manufactured solution mesh sweep\n";
    std::cout << "preconditioner h cells velocity_dofs pressure_dofs GMRES "
                 "status gmres_residual velocity_L2 velocity_H1 pressure_L2 "
                 "divergence_L2\n";
    ...
    for (const MeshSweepCase &mesh_case : mesh_cases)
      {
        ...
        print_sweep_row(mesh_case, fbicg_result);
        ...
        print_sweep_row(mesh_case, famg_result);
        ...
        print_sweep_row(mesh_case, famg_bicg_result);
      }
  }
}
      ...
      if (argc == 1)
      ...
      else
        {
          ...
          const std::string output_prefix =
            argc > 2 ? argv[2] :
                       std::string(AFEPACK_DEFAULT_OUTPUT_DIR) +
                         "/oseen_block_lsc_iterative_afepack";

          std::cout << "AFEPack Oseen block LSC with iterative inner "
                       "solvers Taylor-Hood manufactured solution\n";

          std::vector<OseenLSCIterativeResult> results;
          results.push_back(
            solve_oseen_block_lsc_iterative(
              mesh_file,
              output_prefix,
          ...
          std::cout << "Wrote " << output_prefix
                    << "_{fbicg_lsc,famg_lsc,famg_bicg_lsc}_{ux,uy,p}.dx\n";
        }
    }
