/**
 * @file navier_stokes_driven_cavity_equal_order_stabilized_afepack.cpp
...
#define main navier_stokes_driven_cavity_afepack_hidden_main
#include "navier_stokes_driven_cavity_afepack.cpp"
#undef main
...
namespace
{
  ...
  struct StabilizedNavierStokesCavityResult
  {
  ...
  double pressure_centerline_max_abs_value(
  ...
  double pressure_antisymmetry_rms_value(
    const FEMFunction<double, dim> &pressure)
  {
    const auto &space = pressure.femSpace();
  ...
  StabilizedNavierStokesCavityResult solve_stabilized_viscosity_step(
    const EasyMesh &mesh,
    const FEMSpace<double, dim> &velocity_space,
    const FEMSpace<double, dim> &pressure_space,
    const double viscosity,
    ...
    FEMFunction<double, dim> &p_h)
  {
    ...
    return {viscosity,
            ...
            pressure_centerline_max_abs_value(p_h),
            pressure_antisymmetry_rms_value(p_h),
            divergence_l2_norm(u_x, u_y)};
  }

  std::vector<StabilizedNavierStokesCavityResult>
  solve_stabilized_cavity_continuation(
    ...
    const std::vector<double> &viscosities)
  {
    ...
    std::vector<StabilizedNavierStokesCavityResult> results;
...
int main(int argc, char **argv)
{
  ...
  const std::string output_prefix =
    argc > 2 ? argv[2] :
               std::string(AFEPACK_DEFAULT_OUTPUT_DIR) +
                 "/navier_stokes_driven_cavity_equal_order_stabilized_afepack";
  ...
  try
    {
      const auto results =
        solve_stabilized_cavity_continuation(mesh_file,
                                             output_prefix,
                                             viscosities);

      std::cout << std::scientific << std::setprecision(6);
      std::cout << "AFEPack Navier-Stokes P1/P1 local-projection "
                   "stabilized regularized driven cavity continuation "
                   "(book Example 7.1.3)\n";
