/**
 * @file navier_stokes_pcd_iterative_afepack.cpp
...
namespace
{
  ...
  // The nonlinear tolerance is matched to the inexact PCD block solves below.
  ...
  const std::string output_prefix =
    argc > 2 ? argv[2] :
               std::string(AFEPACK_DEFAULT_OUTPUT_DIR) +
                 "/navier_stokes_pcd_iterative_afepack";

  try
    {
      std::cout << std::scientific << std::setprecision(6);
      std::cout << "AFEPack Navier-Stokes Picard PCD with iterative inner "
                   "solvers Taylor-Hood manufactured solution\n";

      std::vector<NavierStokesPCDResult> results;
      results.push_back(
        solve_navier_stokes_pcd(mesh_file,
                                output_prefix,
      ...
      std::cout << "Wrote " << output_prefix
                << "_{fbicg_pcd,famg_pcd,famg_bicg_pcd}_{ux,uy,p}.dx\n";
    }
