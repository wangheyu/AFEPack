/**
 * @file navier_stokes_contracting_channel_geometry_compare_afepack.cpp
...
namespace
{
  ...
  std::vector<GeometryCase> default_geometry_cases()
  {
    ...
    return {{"linear_trapezoid_proxy",
             ...
             output_dir + "/navier_stokes_contracting_geometry_linear",
             ContractingChannelGeometry::linear_proxy,
             6.0,
             0.5},
            {"quadratic_bow_tie_like",
             curved_mesh_file,
             output_dir + "/navier_stokes_contracting_geometry_quadratic",
...
int main(int argc, char **argv)
{
  if (argc > 3)
    {
      std::cerr << "usage: " << argv[0] << " [viscosities [output-prefix]]\n";
      return 2;
    }
  ...
  const std::string output_prefix =
    argc > 2 ? argv[2] :
               std::string(AFEPACK_DEFAULT_OUTPUT_DIR) +
                 "/navier_stokes_contracting_channel_geometry_compare";

  try
    {
      std::cout << std::scientific << std::setprecision(6);
      std::cout << "AFEPack Navier-Stokes contracting-channel geometry "
                   "comparison (book Exercise 7.6 / Exercise 5.7 proxy)\n";
      std::cout << "geometry nu inverse_nu cells velocity_dofs pressure_dofs "
                   "area outlet_half_width Picard final_update x0_flow "
                   "x1_flow x4_flow flux_imbalance x4_center_ux x4_max_ux "
      ...
      for (const auto &geometry_case : default_geometry_cases())
        {
          ...
          const auto results =
            solve_contracting_channel_continuation(
              geometry_case.mesh_file,
              output_prefix + "_" + geometry_case.name,
              viscosities);

          for (const auto &result : results)
            print_result_row(geometry_case, result);
        }
    }
