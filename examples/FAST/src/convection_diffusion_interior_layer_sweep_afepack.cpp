/**
 * @file convection_diffusion_interior_layer_sweep_afepack.cpp
 * @brief AFEPack 对流扩散方程算例：内层算例。
 *
 * @details
 ...
 *
 * 主要演示内容：
 * - 内层算例：展示解在区域内部急剧变化时的网格分辨率需求和稳定化策略。
 *
...
#define main convection_diffusion_interior_hidden_main
#include "convection_diffusion_interior_layer_afepack.cpp"
#undef main

namespace
{
  enum class StreamlineSweepDirection
  {
    downstream,
    upstream
  };

  enum class InteriorPreconditioner
  {
    none,
    jacobi,
    downstream_gs,
    upstream_gs,
    symmetric_streamline_gs
  };

  struct InteriorSweepResult
  {
  ...
  const char *
  preconditioner_name(const InteriorPreconditioner preconditioner)
  {
    switch (preconditioner)
      {
        case InteriorPreconditioner::none:
          return "none";
        case InteriorPreconditioner::jacobi:
          return "Jacobi";
        case InteriorPreconditioner::downstream_gs:
          return "downstream-GS";
        case InteriorPreconditioner::upstream_gs:
          return "upstream-GS";
        case InteriorPreconditioner::symmetric_streamline_gs:
          return "symmetric-streamline-GS";
      }
    return "unknown";
  }

  double streamline_coordinate(const Point<dim> &point)
  {
    const double p[dim] = {point[0], point[1]};
    return wind_x(p) * point[0] + wind_y(p) * point[1];
  }

  std::vector<int> dof_order(
    const FEMSpace<double, dim> &space,
    const StreamlineSweepDirection direction)
  {
    ...
    std::sort(order.begin(),
              ...
              [&](const int left, const int right) {
                ...
                const double sl = streamline_coordinate(pl);
                const double sr = streamline_coordinate(pr);

                if (std::abs(sl - sr) > 1.0e-12)
                  {
                    const bool increasing =
                      direction == StreamlineSweepDirection::downstream;
                    return increasing ? sl < sr : sl > sr;
                  }
  ...
  InteriorSweepResult run_interior_sweep_case(
    const std::string &mesh_file,
    const double h,
    const double epsilon,
    const InteriorPreconditioner preconditioner)
  {
    ...
    DirectionalGaussSeidelPreconditioner downstream_sweep(
      matrix,
      {dof_order(fem_space, StreamlineSweepDirection::downstream)});
    DirectionalGaussSeidelPreconditioner upstream_sweep(
      matrix,
      {dof_order(fem_space, StreamlineSweepDirection::upstream)});
  ...
  void print_results(const std::vector<InteriorSweepResult> &results)
  {
    std::cout << std::scientific << std::setprecision(6);
    std::cout << "AFEPack interior/downstream layer SUPG sweep "
                 "preconditioner test\n";
    std::cout << "Book Example 3.1.3; beta=(-1,sqrt(3)), so increasing "
                 "beta dot x is the downstream sweep.\n";
  ...
  try
    {
      ...
      const std::vector<InteriorPreconditioner> preconditioners = {
        InteriorPreconditioner::none,
        InteriorPreconditioner::jacobi,
        InteriorPreconditioner::downstream_gs,
        InteriorPreconditioner::upstream_gs,
        InteriorPreconditioner::symmetric_streamline_gs};

      std::vector<InteriorSweepResult> results;
      for (const double epsilon : epsilons)
        for (const auto preconditioner : preconditioners)
          results.push_back(run_interior_sweep_case(mesh_file,
                                                    0.05,
                                                    epsilon,
                                                    preconditioner));
