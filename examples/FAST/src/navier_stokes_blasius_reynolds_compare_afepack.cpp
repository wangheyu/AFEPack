/**
 * @file navier_stokes_blasius_reynolds_compare_afepack.cpp
 * @brief AFEPack 不可压 Navier-Stokes 方程算例：Blasius 雷诺数对比。
 *
 * @details
 ...
 *
 * 主要演示内容：
 * - Blasius 雷诺数对比：扫描雷诺数并比较边界层代理解的变化趋势。
...
namespace
{
  struct BlasiusReynoldsMeshCase
  {
    std::string name;
  ...
  struct BlasiusReynoldsRow
  {
    BlasiusReynoldsMeshCase mesh_case;
    BlasiusResult           result;
  };

  std::vector<BlasiusReynoldsMeshCase> default_mesh_cases()
  {
    ...
    const std::string generated_mesh_file =
      mesh_dir + "/generated_blasius_plate_reynolds_stretched";
    ...
    return {{"easymesh_h0p25",
             ...
             output_dir + "/navier_stokes_blasius_reynolds_easymesh"},
            {"generated_stretched",
             generated_mesh_file,
             output_dir + "/navier_stokes_blasius_reynolds_stretched"}};
  }
  ...
  void print_result_row(const BlasiusReynoldsRow &row)
  {
...
int main(int argc, char **argv)
{
  ...
  try
    {
      std::vector<BlasiusReynoldsRow> rows;
      ...
      std::cout << "AFEPack Navier-Stokes Blasius thin-plate proxy "
                   "Reynolds comparison (book Example 7.1.4 / Exercise 7.2)\n";
