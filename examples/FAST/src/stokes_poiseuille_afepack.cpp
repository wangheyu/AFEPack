/**
 * @file stokes_poiseuille_afepack.cpp
 * @brief AFEPack 不可压 Stokes 方程算例：Poiseuille 管道流。
 *
 * @details
...
namespace
{
  ...
  {
    return 8.0 * viscosity * (1.0 - p[0]);
  }

  std::vector<double> pressure_gradient(const double *p)
  {
    (void)p;
    return {-8.0 * viscosity, 0.0};
  }
                                 ...
                                 const int dof)
  {
    ...
    return std::abs(x) < boundary_tolerance ||
           std::abs(y) < boundary_tolerance ||
           std::abs(y - 1.0) < boundary_tolerance;
  }
  ...
  try
    {
      ...
      std::cout << "AFEPack Stokes Poiseuille reference flow\n";
      ...
      std::cout << "pressure L2 error with pinned first pressure dof: "
