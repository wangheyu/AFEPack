/**
 * @file stokes_block_iterative_afepack.cpp
 * @brief AFEPack 不可压 Stokes 方程算例：块迭代求解。
 *
 * @details
 ...
 *
 * 主要演示内容：
 * - 块迭代求解：使用速度块、压力块和 Schur 补近似构造鞍点系统迭代方法。
  ...
  const std::string output_prefix =
    argc > 2 ? argv[2] :
               std::string(AFEPACK_DEFAULT_OUTPUT_DIR) +
                 "/stokes_block_iterative_afepack";
  ...
  try
    {
      ...
      std::cout << "AFEPack Stokes sparse block GMRES Taylor-Hood "
