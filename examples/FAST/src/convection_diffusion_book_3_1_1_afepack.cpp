/**
 * @file convection_diffusion_book_3_1_1_afepack.cpp
 * @brief AFEPack 对流扩散方程算例：教材例题 3.1.1。
 *
 * @details
 ...
 *
 * 主要演示内容：
 * - 教材例题 3.1.1：复现实验教材中的典型参数和边界条件，便于与理论公式逐项对照。
 *
...
{
  constexpr int    dim = 2;
  constexpr double beta_x = 0.0;
  constexpr double beta_y = 2.0;
  constexpr double lower_strip_y_max = 0.875;
  ...
  {
    const double x_book = 2.0 * p[0] - 1.0;
    const double denominator =
      1.0 - std::exp(-2.0 / current_epsilon);
    const double layer =
      std::exp(2.0 * (p[1] - 1.0) / current_epsilon);
    return x_book * (1.0 - layer) / denominator;
  }
  ...
  std::vector<double> exact_gradient(const double *p)
  {
    ...
    return {2.0 * (1.0 - layer) / denominator,
            -2.0 * x_book * layer /
              (current_epsilon * denominator)};
  }
  ...
  std::pair<double, double> nodal_range(
    const FEMFunction<double, dim> &solution)
  {
    ...
    for (std::size_t i = 0; i < solution.size(); ++i)
      {
        minimum = std::min(minimum, solution(i));
        maximum = std::max(maximum, solution(i));
      }

    return {minimum, maximum};
  }

  double lower_strip_h1_semi_error(
    const FEMFunction<double, dim> &solution)
  {
    ...
    for (int e = 0; e < space.n_element(); ++e)
      {
        ...
        const auto q_point =
          element.local_to_global(quad_info.quadraturePoint());
        const auto gradient = solution.gradient(q_point, element);

        for (int q = 0; q < quad_info.n_quadraturePoint(); ++q)
          {
            if (q_point[q][1] > lower_strip_y_max)
              continue;

            const auto exact = exact_gradient(q_point[q]);
            const double dx = gradient[q][0] - exact[0];
            const double dy = gradient[q][1] - exact[1];
            const double jxw =
              quad_info.weight(q) * jacobian[q] * volume;
            error_square += jxw * (dx * dx + dy * dy);
          }
      }

    return std::sqrt(error_square);
  }
                      ...
                      const bool write_solution)
  {
    ...
    const auto range = nodal_range(solution);
    return {experiment,
            mesh_label,
            nominal_h,
            epsilon,
            beta_norm() * nominal_h / (2.0 * epsilon),
            use_supg,
            ...
            lower_strip_h1_semi_error(solution),
            range.first,
            range.second};
  }
  ...
  void print_results(const std::vector<CaseResult> &results)
  {
    ...
    std::cout << "AFEPack convection-diffusion book Example 3.1.1\n";
    std::cout << "Book domain (-1,1)^2 is mapped to the unit square; "
              << "the mapped wind is beta=(0,2), f=0.\n";
    std::cout << std::setw(12) << "experiment"
              << std::setw(10) << "mesh" << std::setw(14) << "epsilon"
              << std::setw(14) << "Pe_h" << std::setw(10) << "method"
              << std::setw(10) << "cells" << std::setw(10) << "dofs"
              << std::setw(8) << "iter" << std::setw(14) << "relres"
              << std::setw(14) << "L2" << std::setw(14) << "H1-semi"
              << std::setw(14) << "H1-low" << std::setw(14) << "min"
              << std::setw(14) << "max"
              << '\n';

    for (const auto &result : results)
      {
        std::cout << std::setw(12) << result.experiment
                  << std::setw(10) << result.mesh_label
                  << std::setw(14) << result.epsilon
                  << std::setw(14) << result.peclet
                  ...
                  << std::setw(14) << result.lower_strip_h1_error
                  << std::setw(14) << result.minimum
...
int main(int argc, char **argv)
{
  if (argc > 3)
    {
      std::cerr << "usage: " << argv[0] << " [mesh-dir [output-prefix]]\n";
      return 2;
    }
  ...
  const std::string output_prefix =
    argc > 2 ? argv[2] :
               std::string(AFEPACK_DEFAULT_OUTPUT_DIR) +
                 "/convection_diffusion_book_3_1_1_afepack";
  ...
  try
    {
      ...
      const auto mesh_path = [&mesh_dir](const std::string &label) {
        return mesh_dir + "/unit_square_" + label;
      };

      const MeshCase fixed_mesh{
        "h0p05", mesh_path("h0p05"), 0.05};
      const std::vector<EpsilonCase> epsilons = {
        {"eps1o16", 1.0 / 16.0},
        {"eps1o64", 1.0 / 64.0},
        {"eps1o256", 1.0 / 256.0}};
      const std::vector<MeshCase> meshes = {
        {"h0p20", mesh_path("h0p20"), 0.20},
        {"h0p10", mesh_path("h0p10"), 0.10},
        {"h0p05", mesh_path("h0p05"), 0.05},
        {"h0p025", mesh_path("h0p025"), 0.025}};

      std::vector<CaseResult> results;
      for (const auto &epsilon : epsilons)
        {
          results.push_back(run_case(fixed_mesh.file,
                                     "eps-sweep",
                                     fixed_mesh.label,
                                     fixed_mesh.h,
                                     output_prefix + "_" +
                                       epsilon.label + "_galerkin.dx",
                                     epsilon.value,
                                     false,
                                     false));
          results.push_back(run_case(fixed_mesh.file,
                                     ...
                                     fixed_mesh.h,
                                     output_prefix + "_" +
                                       epsilon.label + "_supg.dx",
                                     epsilon.value,
                                     true,
                                     epsilon.label == "eps1o256"));
        }

      for (const auto &mesh : meshes)
        {
          const double epsilon = 1.0 / 64.0;
          results.push_back(run_case(mesh.file,
                                     "h-sweep",
                                     mesh.label,
                                     mesh.h,
                                     output_prefix + "_" + mesh.label +
                                       "_galerkin.dx",
                                     epsilon,
                                     false,
                                     false));
      ...
      std::cout << "Wrote " << output_prefix
                << "_eps1o256_supg.dx and " << output_prefix
                << "_h0p025_supg.dx\n";
    }
