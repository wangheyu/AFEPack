/**
 * @file convection_diffusion_characteristic_layer_afepack.cpp
 * @brief AFEPack 对流扩散方程算例：特征层算例。
 *
 * @details
 ...
 *
 * 主要演示内容：
 * - 特征层算例：展示沿对流方向传播的特征层结构以及标准 Galerkin 离散的响应。
 *
...
namespace
{
  ...
  double current_epsilon = 1.0 / 200.0;
  ...
  double boundary_inflow(const double *p)
  {
    (void)p;
    return 1.0;
  }

  // Example 3.1.2 is posed on (-1,1)^2. This program uses the equivalent
  // unit-square coordinates X=2x-1, Y=2y-1. The side-wall data therefore
  // decrease from one at y=0 to zero at y=1, quadratically on the right and
  // cubically on the left.
  double wall_coordinate(const double *p)
  {
    return std::clamp(1.0 - p[1], 0.0, 1.0);
  }

  double boundary_right_wall(const double *p)
  {
    const double s = wall_coordinate(p);
    return s * s;
  }

  double boundary_left_wall(const double *p)
  {
    const double s = wall_coordinate(p);
    return s * s * s;
  }
                      ...
                      const bool write_solution)
  {
    ...
    const SolveInfo solve_info =
      solve_gmres(matrix, solution, rhs, 1.0e-9, 50, 2000);
  ...
  void print_results(const std::vector<CaseResult> &results)
  {
    ...
    std::cout << "AFEPack convection-diffusion P1 characteristic boundary layers\n";
  ...
  const std::string output_prefix =
    argc > 2 ? argv[2] :
               std::string(AFEPACK_DEFAULT_OUTPUT_DIR) +
                 "/convection_diffusion_characteristic_layer_afepack";
