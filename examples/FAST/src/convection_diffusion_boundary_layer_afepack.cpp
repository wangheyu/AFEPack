/**
 * @file convection_diffusion_boundary_layer_afepack.cpp
...
namespace
{
  ...
  {
    const double exp_minus_inverse = std::exp(-1.0 / current_epsilon);
    const double denominator = 1.0 - exp_minus_inverse;
    const double layer = std::exp((p[0] - 1.0) / current_epsilon);
    return p[0] - (layer - exp_minus_inverse) / denominator;
  }
  ...
  std::vector<double> exact_gradient(const double *p)
  {
    ...
    const double denominator = 1.0 - exp_minus_inverse;
    const double layer = std::exp((p[0] - 1.0) / current_epsilon);
    return {1.0 - layer / (current_epsilon * denominator), 0.0};
  }
  ...
  void print_results(const std::vector<CaseResult> &results)
  {
    ...
    std::cout << "AFEPack convection-diffusion P1 exponential boundary layer\n";
  ...
  const std::string output_prefix =
    argc > 2 ? argv[2] :
               std::string(AFEPACK_DEFAULT_OUTPUT_DIR) +
                 "/convection_diffusion_boundary_layer_afepack";
