/**
 * @file convection_diffusion_interior_layer_afepack.cpp
...
namespace
{
  ...
  double boundary_bottom(const double *p)
  {
    return p[0] < 0.5 ? 0.0 : 1.0;
  }

  double boundary_zero(const double *p)
  {
    (void)p;
    return 0.0;
  }

  double boundary_one(const double *p)
  {
    (void)p;
    return 1.0;
  }

  double reduced_streamline_level(const double *p)
  {
    return p[1] + sqrt3 * p[0] - 0.5 * sqrt3;
  }
  ...
  void print_results(const std::vector<CaseResult> &results)
  {
    ...
    std::cout << "AFEPack convection-diffusion P1 interior and downstream layers\n";
    ...
    std::cout << "Reduced discontinuity in unit-square coordinates: "
              << "y + sqrt(3) x = sqrt(3)/2\n";
  }
}
  ...
  const std::string output_prefix =
    argc > 2 ? argv[2] :
               std::string(AFEPACK_DEFAULT_OUTPUT_DIR) +
                 "/convection_diffusion_interior_layer_afepack";
