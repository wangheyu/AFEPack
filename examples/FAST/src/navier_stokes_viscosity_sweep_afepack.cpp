/**
 * @file navier_stokes_viscosity_sweep_afepack.cpp
...
namespace
{
  ...
  std::vector<SweepResult> solve_viscosity_sweep(
    ...
    const std::string &output_prefix)
  {
    ...
    const std::vector<double> viscosities = {1.0, 0.5, 0.25, 0.125, 0.0625};
    ...
    for (const double nu : viscosities)
      {
        ...
        for (; picard_iteration < max_picard_iterations;
             ++picard_iteration)
          {
            ...
            if (!gmres_info.converged)
              throw std::runtime_error(
                "right-preconditioned GMRES failed for viscosity " +
  ...
  const std::string output_prefix =
    argc > 2 ? argv[2] :
               std::string(AFEPACK_DEFAULT_OUTPUT_DIR) +
                 "/navier_stokes_viscosity_sweep_afepack";

  try
    {
      const auto results = solve_viscosity_sweep(mesh_file, output_prefix);

      std::cout << std::scientific << std::setprecision(6);
      std::cout << "AFEPack Navier-Stokes sparse block GMRES Picard "
                   "viscosity continuation manufactured solution\n";
      if (!results.empty())
        {
      ...
      std::cout << "nu Re system_nonzeros Picard GMRES GMRES/Picard update "
