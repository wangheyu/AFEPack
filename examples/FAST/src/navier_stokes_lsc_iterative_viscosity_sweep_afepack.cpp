/**
 * @file navier_stokes_lsc_iterative_viscosity_sweep_afepack.cpp
...
namespace
{
  ...
  constexpr double lsc_linear_tolerance = 1.0e-4;
  ...
  {
    constexpr VelocityInverseKind velocity_inverse_kind =
            ...
            if (!gmres_info.converged)
              {
                ...
                message << std::scientific << std::setprecision(6)
                        << "right-preconditioned LSC AMG-BiCG GMRES failed "
  ...
  const std::string output_prefix =
    argc > 2 ? argv[2] :
               std::string(AFEPACK_DEFAULT_OUTPUT_DIR) +
                 "/navier_stokes_lsc_iterative_viscosity_sweep_afepack";
  ...
  try
    {
      ...
      std::cout << "AFEPack Navier-Stokes Picard LSC AMG-BiCG iterative "
