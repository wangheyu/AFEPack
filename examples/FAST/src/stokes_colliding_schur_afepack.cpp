/**
 * @file stokes_colliding_schur_afepack.cpp
...
namespace
{
  ...
  double exact_velocity_x(const double *p)
  {
  ...
  double exact_velocity_y(const double *p)
  {
    const double x = original_x(p);
    const double y = original_y(p);
  ...
  std::vector<double> exact_velocity_x_gradient(const double *p)
  {
    const double x = original_x(p);
    const double y = original_y(p);
  ...
  std::vector<double> exact_velocity_y_gradient(const double *p)
  {
    const double x = original_x(p);
    const double y = original_y(p);
  ...
  double exact_pressure(const double *p)
  {
    const double x = original_x(p);
    const double y = original_y(p);
  ...
  {
    return component == 0 ? exact_velocity_x(p) : exact_velocity_y(p);
  }
  ...
  void apply_homogeneous_velocity_dirichlet_boundary(
        ...
        for (int q = 0; q < quad_info.n_quadraturePoint(); ++q)
          {
            ...
            const double error = values[q] - exact_pressure(point);
            area += jxw;
                            ...
                            const int max_gmres_iterations)
  {
    ...
    const auto &pressure_pin_point_info =
      pressure_space.dofInfo(pressure_pin).interp_point;
    const double pressure_pin_point[dim] = {pressure_pin_point_info[0],
                                            pressure_pin_point_info[1]};
    apply_dirichlet_constraint(matrix,
                               rhs,
                               pressure_index(pressure_pin,
                                              n_velocity_dofs),
                               exact_pressure(pressure_pin_point));
        ...
        case PreconditionerKind::schur_mass_amg2:
          {
            ...
            apply_homogeneous_velocity_dirichlet_boundary(
              velocity_stiffness,
    ...
    FunctionFunction<double> exact_u_x(&exact_velocity_x,
                                       &exact_velocity_x_gradient);
    FunctionFunction<double> exact_u_y(&exact_velocity_y,
                                       &exact_velocity_y_gradient);
  ...
  const std::string output_prefix =
    argc > 2 ? argv[2] :
               std::string(AFEPACK_DEFAULT_OUTPUT_DIR) +
                 "/stokes_colliding_schur_afepack";
  ...
  try
    {
      ...
      std::cout << "AFEPack Stokes colliding-flow Schur/AMG block "
                   "preconditioner comparison (book Example 5.1.4)\n";
      std::cout << "The polynomial solution is mapped from (-1,1)^2 "
                   "to the unit square; pressure is compared modulo "
                   "constants.\n";
      ...
      std::cout << "preconditioner cells velocity_dofs pressure_dofs "
                   ...
                   "velocity_H1 pressure_L2_mod_constant divergence_L2\n";
