/**
 * @file navier_stokes_lsc_afepack.cpp
 * @brief AFEPack 不可压 Navier-Stokes 方程算例：LSC 预条件器。
 *
 * @details
  ...
  const std::string output_prefix =
    argc > 2 ? argv[2] :
               std::string(AFEPACK_DEFAULT_OUTPUT_DIR) +
                 "/navier_stokes_lsc_afepack";

  try
    {
      const NavierStokesLSCResult result =
        solve_navier_stokes_lsc(mesh_file, output_prefix);

      std::cout << std::scientific << std::setprecision(6);
      std::cout << "AFEPack Navier-Stokes Picard LSC Taylor-Hood manufactured "
                   "solution\n";
