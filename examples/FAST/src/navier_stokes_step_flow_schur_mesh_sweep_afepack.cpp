/**
 * @file navier_stokes_step_flow_schur_mesh_sweep_afepack.cpp
 * @brief AFEPack 不可压 Navier-Stokes 方程算例：网格尺度扫描。
 *
 * @details
...
namespace
{
  ...
  struct StepFlowSchurMeshSweepRow
  {
    StepFlowSchurMeshCase mesh_case;
    StepFlowResult        result;
  };

  std::vector<StepFlowSchurMeshCase> default_mesh_cases()
  {
    ...
    return {{"step_channel_h0p50",
             ...
             output_dir + "/navier_stokes_step_flow_schur_mesh_sweep_h0p50",
             0.50},
            {"step_channel_h0p25",
             mesh_dir + "/step_channel_h0p25",
             output_dir + "/navier_stokes_step_flow_schur_mesh_sweep_h0p25",
             0.25}};
  }

  void print_result_row(const StepFlowSchurMeshSweepRow &row)
  {
...
int main(int argc, char **argv)
{
  ...
  const auto viscosities = parse_viscosities(
    argc > 1 ? argv[1] : "1,0.5,0.25,0.125,0.05,0.02");

  try
    {
      std::vector<StepFlowSchurMeshSweepRow> rows;
      for (const auto &mesh_case : default_mesh_cases())
        {
          const auto results = solve_step_flow_continuation(
            mesh_case.mesh_file, mesh_case.output_prefix, viscosities);
          for (const auto &result : results)
            rows.push_back({mesh_case, result});
        }

      std::cout << std::scientific << std::setprecision(6);
      std::cout << "AFEPack Navier-Stokes expansion step-flow Schur/AMG "
                   "mesh/viscosity sweep (book Example 7.1.2 and "
                   "Exercise 7.5)\n";
      ...
      std::cout << "mesh h nu Re cells velocity_dofs pressure_dofs "
                   ...
                   "velocity_L2 velocity_H1 max_speed inflow outflow "
